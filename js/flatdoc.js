...
function($) {
	...
	Highlighters.php = function(code) {
		return code.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/("[^\"]*?")/g, '<span class="string">$1</span>').replace(/('[^\']*?')/g, '<span class="string">$1</span>').replace(/\/\/(.*)/g, '<span class="comment">//$1</span>').replace(/&lt;([^!][^\s&]*)/g, '&lt;<span class="keyword">$1</span>').replace(/\b(public|function|new|class |for|while|if)\b/gm, '<span class="init">$1</span>').replace(/(\$\w+)/g, '<span class="keyword">$1</span>')
	};	
